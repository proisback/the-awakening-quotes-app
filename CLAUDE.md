...
- `index.html` — an inline SVG icon sprite (`<symbol id="i-…">`, stroke icons styled via `.ic-svg`), a first-open splash (`#splash`), four screens (reader feed, browse, saved, settings) + bottom tab bar, a note `<dialog>`, a share `<dialog>` (`#shareSheet`: Save as image / Share as text), a premium paywall `<dialog>` (`#paywall` — opened by locked theme packs/fonts and the Get Premium row), and a `#toast` element.
